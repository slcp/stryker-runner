...
const mockStrykerCommand = strykerCommand as jest.MockedFn<
  typeof strykerCommand
>;
const mockStrykerConfigFilePath = strykerConfigFilePath as jest.MockedFn<
  typeof strykerConfigFilePath
>;
    ...
    describe("Curried command runner function", () => {
      it("should execute a Stryker command with a custom config file path", () => {
        ...
        expect(mockRunCommandReturn).toHaveBeenCalledWith(
          "a command run --mutate /path/to/file:1-10 a path"
        );
      });
